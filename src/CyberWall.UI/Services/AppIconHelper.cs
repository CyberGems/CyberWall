...
public static class AppIconHelper
{
    ...
    {
        if (_cachedIcon != null && size == 32) return _cachedIcon;
        ...
        DestroyIcon(hIcon);
        if (size == 32) _cachedIcon = icon;
        return icon;
    }

    public static WpfImageSource CreateShieldImageSource(int size = 64)
    {
        if (_cachedImageSource != null && size == 64) return _cachedImageSource;
        using var icon = CreateShieldIcon(size);
        ...
        bi.EndInit();
        bi.Freeze();
        if (size == 64) _cachedImageSource = bi;
        return bi;
    }
